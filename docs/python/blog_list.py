...
path = os.path.join(os.path.dirname(__file__), '..', 'blog', 'md', 'post6.md')
document = open(path, encoding='utf8')
content = document.read().replace('\n', '\\n').replace('`', '\`').replace('</script', '<\/script').replace("'", "\'")
document.close()

blogpost.open_blog('Switching to HTTPS for Sites Hosted on AWS', content)
...
post6 = blog_object.Blog('Switching to HTTPS for Sites Hosted on AWS', 'How to register SSL Certificate on AWS', 'AWS', 'img/post6.png', 'blog/Switching to HTTPS for Sites Hosted on AWS.html')
